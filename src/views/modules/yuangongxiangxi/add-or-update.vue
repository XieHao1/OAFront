    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="员工手机" prop="yuangongshouji">
          <el-input v-model="ruleForm.yuangongshouji" 
              placeholder="员工手机" clearable  :readonly="ro.yuangongshouji"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="员工手机" prop="yuangongshouji">
              <el-input v-model="ruleForm.yuangongshouji" 
                placeholder="员工手机" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="员工地址" prop="yuangongdizhi">
          <el-input v-model="ruleForm.yuangongdizhi" 
              placeholder="员工地址" clearable  :readonly="ro.yuangongdizhi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="员工地址" prop="yuangongdizhi">
              <el-input v-model="ruleForm.yuangongdizhi" 
                placeholder="员工地址" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="婚否" prop="hunfou">
          <el-select :disabled="ro.hunfou" v-model="ruleForm.hunfou" placeholder="请选择婚否">
            <el-option
                v-for="(item,index) in hunfouOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="婚否" prop="hunfou">
	      <el-input v-model="ruleForm.hunfou"
                placeholder="婚否" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhaopian = true;
          ...
          if(o=='yuangongdizhi'){
            this.ruleForm.yuangongdizhi = obj[o];
	    this.ro.yuangongdizhi = true;
            continue;
          }
          if(o=='minzu'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.hunfouOptions = "未婚,已婚".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `yuangongxiangxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yuangongxiangxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yuangongxiangxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
